...
def clean(string):
	town_data = [town.split(":") for town in string.split("\n")]
	for k in range(len(town_data)):
		town_data[k][1] = town_data[k][1].split(",")
	town_data_dict = {}
	for town in town_data:
		data = []
		for month in town[1]:
			data.append(float(month.split(" ")[1]))
		town_data_dict[town[0]] = data
	return town_data_dict

def mean(town, string):
	town_data = clean(string)
	if town not in town_data.keys():
		return -1
	return sum(town_data[town]) / len(town_data[town])
...
def variance(town, string):
	...
	if average == -1:
		return average
	town_data = clean(string)
	return sum([(data - average)**2 for data in town_data[town]]) / len(town_data[town])
...
if __name__ == "__main__":
	...
	assert mean("London", data) == 51.199999999999996
	assert mean("Beijing", data) == 52.416666666666664
	assert variance("London", data) == 57.42833333333374
	assert variance("Beijing", data) == 4808.37138888889 